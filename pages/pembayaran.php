...
$dibayar=mysql_fetch_array(mysql_query("SELECT sum(jumlah_bayar) as total from pembayaran where kode_pembelian='$_GET[id]'"));
 ?>
      ...
      <div class="row">
        <div class="col-xs-6">
          <h4 style="color: #248dca">Rincian Pembayaran</h4>
          <table class="table table-border">
            ...
            <?php 
            $no=1;
            $bay=mysql_query("SELECT * from pembayaran where kode_pembelian='$_GET[id]'");
            while ($q2=mysql_fetch_array($bay)) :
             ?>
            <tr>
              <td><?php echo $no++; ?></td>
              <td><?php echo  $q2['tgl_pembayaran'] ?></td>
              <td><?php echo  format_rupiah($q2['jumlah_bayar']) ?></td>
              <td><?php echo  $q2['ket'] ?></td>
            </tr>
          <?php endwhile ?>
            </tbody>
          </table>
          <?php if (mysql_num_rows($bay)<1) {
            echo "<i>Belum ada transaksi pembayaran. </i>";
          } ?>
        </div>
      </div>
